...
class Debugger(object):
    def __init__(self, ipynb=False, theme="black", num_classes=-1, dataset=None, down_ratio=4, class_names=None, opt=None):
        self.ipynb = ipynb
        if not self.ipynb:
        ...
        self.names = None
        if class_names:
            self.names = [str(n) for n in class_names]
        ...
        if dataset == "coco_hp":
        ...
        elif dataset == "yolo_dataset":
            # 为YOLO数据集配置关键点可视化
            self.num_class = opt.num_classes if opt and hasattr(opt, 'num_classes') else 1
            self.num_joints = opt.num_joints if opt and hasattr(opt, 'num_joints') else 0

            # 设置类别名称
            if self.names is None:
                if self.num_class > 0:
                    self.names = ["class_{}".format(i) for i in range(self.num_class)]
                else:
                    self.names = ["object"]
            ...
            if self.num_joints > 0:
                ...
                # 创建连接线颜色
                self.ec = [(200, 200, 200) for _ in range(len(self.edges))]
        else:
            if dataset == "gta":
            ...
            if self.names is None and (num_classes == 80 or dataset == "coco"):
                self.names = coco_class_name
            elif self.names is None and (num_classes == 20 or dataset == "pascal"):
                self.names = pascal_class_name
            elif self.names is None and (num_classes == 3 or dataset == "kitti"):
                self.names = kitti_class_name
            elif self.names is None:
                if num_classes > 0:
                    self.names = ["class_{}".format(i) for i in range(num_classes)]
                else:
                    self.names = ["object"]
    ...
    def add_mask(self, mask, bg, imgId="default", trans=0.8):
        self.imgs[imgId] = (mask.reshape(mask.shape[0], mask.shape[1], 1) * 255 * trans + bg * (1 - trans)).astype(
            np.uint8
        )
  ...
  """
    ...
    def gen_colormap_hp(self, img, output_res=None):
        # Apply sigmoid to match decoding logic
        img = 1 / (1 + np.exp(-img))
        ...
        # Find and mark max values for each joint
        for j in range(c):
            joint_heat = img[:, :, j, 0]
            max_val = np.max(joint_heat)
            if max_val > 0.1:  # same threshold as in decode.py
                max_pos = np.unravel_index(np.argmax(joint_heat), joint_heat.shape)
                max_pos = (int(max_pos[1] * output_res[1] / w), 
                          int(max_pos[0] * output_res[0] / h))
                cv2.circle(color_map, max_pos, 3, (0, 0, 255), -1)
                cv2.putText(color_map, f"{max_val:.4f}", 
                          (max_pos[0]+5, max_pos[1]), 
    ...
    def add_coco_bbox(self, bbox, cat, conf=1, show_txt=True, img_id="default"):
        ...
        score_str = "{:.2g}".format(conf)
        txt = "{}{}".format(self.names[cat], score_str)
        ...
        for cat in dets:
            ...
            for i in range(len(dets[cat])):
                if dets[cat][i, -1] > center_thresh:
                    ...
                    cv2.polylines(
                        bird_view, [rect.reshape(-1, 1, 2).astype(np.int32)], True, lc, 2, lineType=cv2.LINE_AA
                    )
                    for e in [[0, 1]]:
                        t = 4 if e == [0, 1] else 1
                        cv2.line(
    ...
    def add_bird_views(self, dets_dt, dets_gt, center_thresh=0.3, img_id="bird"):
        ...
        for ii, (dets, lc, cc) in enumerate(
            [(dets_gt, (12, 49, 250), (0, 0, 255)), (dets_dt, (250, 152, 12), (255, 0, 0))]
        ):
