...
from models.losses import FocalLoss, RegL1Loss, RegLoss, RegWeightedL1Loss, RegBCEWithLogitsLoss
...
class MultiPoseLoss(torch.nn.Module):
  def __init__(self, opt):
    ...
    self.crit_hp_vis = RegBCEWithLogitsLoss() if opt.num_joints > 0 else None
    self.opt = opt

  def forward(self, outputs, batch):
    opt = self.opt
    hm_loss, wh_loss, off_loss = 0, 0, 0
    hp_loss, hm_hp_loss, hp_offset_loss, hp_vis_loss = 0, 0, 0, 0
    for s in range(opt.num_stacks):
      ...
      if self.crit_hp_vis is not None and 'hp_vis' in output:
        hp_vis_loss += self.crit_hp_vis(
          output['hp_vis'], batch['hp_vis_mask'], batch['ind'], batch['hp_vis']) / opt.num_stacks
    loss = opt.hm_weight * hm_loss + opt.wh_weight * wh_loss + \
           opt.off_weight * off_loss + opt.hp_weight * hp_loss + \
           opt.hm_hp_weight * hm_hp_loss + opt.off_weight * hp_offset_loss + \
           opt.hp_vis_weight * hp_vis_loss

    loss_stats = {'loss': loss, 'hm_loss': hm_loss, 'hp_loss': hp_loss, 
                  'hm_hp_loss': hm_hp_loss, 'hp_offset_loss': hp_offset_loss,
                  'hp_vis_loss': hp_vis_loss, 'wh_loss': wh_loss, 'off_loss': off_loss}
...
class MultiPoseTrainer(BaseTrainer):
  ...
  def _get_losses(self, opt):
    loss_states = ['loss', 'hm_loss', 'hp_loss', 'hm_hp_loss', 
                   'hp_offset_loss', 'hp_vis_loss', 'wh_loss', 'off_loss']
  ...
  def debug(self, batch, output, iter_id):
    ...
    dets = multi_pose_decode(
      ...
      reg=reg, hm_hp=hm_hp, hp_offset=hp_offset, hp_vis=output.get('hp_vis'), K=opt.K)
    dets = dets.detach().cpu().numpy().reshape(1, -1, dets.shape[2])

    # 计算关键点数量
    num_joints = opt.num_joints if hasattr(opt, 'num_joints') else 17
    kp_start_idx = 5
    kp_end_idx = kp_start_idx + num_joints * 2

    dets[:, :, :4] *= opt.input_res / opt.output_res
    dets[:, :, kp_start_idx:kp_end_idx] *= opt.input_res / opt.output_res
    dets_gt = batch['meta']['gt_det'].numpy().reshape(1, -1, dets.shape[2])
    dets_gt[:, :, :4] *= opt.input_res / opt.output_res
    dets_gt[:, :, kp_start_idx:kp_end_idx] *= opt.input_res / opt.output_res

    for i in range(1):
      debugger = Debugger(
        dataset=opt.dataset, ipynb=(opt.debug==3), theme=opt.debugger_theme, opt=opt)
      ...
      for k in range(len(dets[i])):
        if dets[i, k, 4] > opt.center_thresh:
          ...
          if num_joints > 0:
            debugger.add_coco_hp(dets[i, k, kp_start_idx:kp_end_idx], img_id='out_pred')
      ...
      for k in range(len(dets_gt[i])):
        if dets_gt[i, k, 4] > opt.center_thresh:
          ...
          if num_joints > 0:
            debugger.add_coco_hp(dets_gt[i, k, kp_start_idx:kp_end_idx], img_id='out_gt')
  ...
  def save_debug_images(self, batch, output, iter_id, epoch, phase):
    """保存调试图像"""
    self.debug(batch, output, f"{phase}_epoch_{epoch}_batch_{iter_id}")
  ...
  def save_result(self, output, batch, results):
    ...
    dets = multi_pose_decode(
      output['hm'], output['wh'], output['hps'],
      reg=reg, hm_hp=hm_hp, hp_offset=hp_offset, hp_vis=output.get('hp_vis'), K=self.opt.K)
    dets = dets.detach().cpu().numpy().reshape(1, -1, dets.shape[2])
