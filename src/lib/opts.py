...
class opts(object):
    def __init__(self):
        ...
        self.parser.add_argument(
            "--debug_aug_vis", action="store_true", help="save augmented samples with keypoints overlay to tmp/"
        )
        self.parser.add_argument(
            "--log_sample_details", action="store_true", help="print detailed dataset sample logs during loading"
        )
        self.parser.add_argument(
            "--val_debug_batches",
            type=int,
            default=1,
            help="save this many validation batches as visualizations each epoch (set 0 to disable)",
        )
        ...
        self.parser.add_argument(
            "--not_cuda_benchmark", action="store_true", help="disable when the input size is not fixed."
        )
        ...
        # ONNX export
        self.parser.add_argument(
            "--export_onnx", action="store_true", help="export trained model to ONNX format during validation"
        )
        self.parser.add_argument(
            "--verify_onnx", action="store_true", help="verify ONNX model output matches PyTorch model"
        )
        self.parser.add_argument(
            "--dynamic_batch", action="store_true", help="export ONNX model with dynamic batch size"
        )
        ...
        self.parser.add_argument(
            ...
            type=int,
            default=-1,
            help="input height and width. -1 for default from " "dataset. Will be overriden by input_h | input_w",
        )
        ...
        self.parser.add_argument(
            "--trainval", action="store_true", help="include validation in training and " "test on test set"
        )
        ...
        self.parser.add_argument(
            "--not_prefetch_test", action="store_true", help="not use parallal data pre-processing."
        )
        self.parser.add_argument(
            "--fix_res", action="store_true", help="fix testing resolution or keep " "the original resolution"
        )
        self.parser.add_argument(
            "--keep_res", action="store_true", help="keep the original resolution" " during validation."
        )

        # dataset
        self.parser.add_argument(
            "--not_rand_crop", action="store_true", help="not use the random crop data augmentation" "from CornerNet."
        )
        self.parser.add_argument(
            "--shift", type=float, default=0.1, help="when not using random crop" "apply shift augmentation."
        )
        self.parser.add_argument(
            "--scale", type=float, default=0.4, help="when not using random crop" "apply scale augmentation."
        )
        self.parser.add_argument(
            "--rotate", type=float, default=0, help="when not using random crop" "apply rotation augmentation."
        )
        ...
        self.parser.add_argument(
            ...
            type=float,
            default=0,
            help="fixed gaussian radius (in output stride) for heatmaps; 0 means use adaptive radius",
        )
        self.parser.add_argument(
            "--yolo_num_kpts",
            type=int,
            default=-1,
            help="number of keypoints per object for YOLO format datasets (-1 to auto-detect, 0 to disable keypoints)",
        )
        self.parser.add_argument(
            "--yolo_flip_pairs",
            type=str,
            default="",
            help="comma separated pairs like 0-1,2-3 defining horizontal flip swaps for YOLO keypoints",
        )
        self.parser.add_argument(
            "--yolo_mean",
            type=str,
            default="",
            help="override YOLO dataset normalization mean (three floats, BGR order, comma/space separated)",
        )
        self.parser.add_argument(
            "--yolo_std",
            type=str,
            default="",
            help="override YOLO dataset normalization std (three floats, BGR order, comma/space separated)",
        )
        self.parser.add_argument(
        ...
        # ddd
        self.parser.add_argument(
            "--aug_ddd", type=float, default=0.5, help="probability of applying crop augmentation."
        )
        self.parser.add_argument(
            "--rect_mask",
            action="store_true",
            help="for ignored object, apply mask on the " "rectangular region or just center point.",
        )
        self.parser.add_argument(
            "--kitti_split", default="3dop", help="different validation split for kitti: " "3dop | subcnn"
        )

        # loss
        self.parser.add_argument(
            "--mse_loss", action="store_true", help="use mse loss or focal loss to train " "keypoint heatmaps."
        )
        # ctdet
        ...
        self.parser.add_argument(
            "--hm_hp_weight", type=float, default=1, help="loss weight for human keypoint heatmap."
        )
        self.parser.add_argument(
            "--hp_vis_weight", type=float, default=1, help="loss weight for keypoint visibility logits."
        )
        # ddd
        ...
        self.parser.add_argument(
            "--scores_thresh", type=float, default=0.1, help="threshold for extreme point heatmap."
        )
        ...
        self.parser.add_argument(
            "--eval_oracle_offset", action="store_true", help="use ground truth local heatmap offset."
        )
        self.parser.add_argument("--eval_oracle_kps", action="store_true", help="use ground truth human pose offset.")
        self.parser.add_argument(
            "--eval_oracle_hmhp", action="store_true", help="use ground truth human joint heatmaps."
        )
        self.parser.add_argument(
            "--eval_oracle_hp_offset", action="store_true", help="use ground truth human joint local offset."
        )
    ...
    def update_dataset_info_and_set_heads(self, opt, dataset):
        ...
        if hasattr(dataset, "get_dataset_spec"):
            dataset_spec = dataset.get_dataset_spec(opt) or {}

        input_h, input_w = dataset_spec.get("default_resolution", dataset.default_resolution)
        opt.mean = dataset_spec.get("mean", dataset.mean)
        opt.std = dataset_spec.get("std", dataset.std)
        opt.num_classes = dataset_spec.get("num_classes", dataset.num_classes)
        if "num_joints" in dataset_spec:
            opt.num_joints = dataset_spec["num_joints"]
        else:
            opt.num_joints = getattr(dataset, "num_joints", 0)
        if "class_names" in dataset_spec:
            opt.class_names = dataset_spec["class_names"]
        if "flip_idx" in dataset_spec:
            opt.flip_idx = dataset_spec["flip_idx"]
        if "keypoint_edges" in dataset_spec:
            opt.keypoint_edges = dataset_spec["keypoint_edges"]
        ...
        if opt.task == "exdet":
            ...
            if not hasattr(opt, "num_joints") or opt.num_joints <= 0:
                opt.num_joints = getattr(dataset, "num_joints", 0)
            opt.flip_idx = getattr(opt, "flip_idx", getattr(dataset, "flip_idx", []))
            opt.heads = {"hm": opt.num_classes, "wh": 2, "hps": max(0, opt.num_joints * 2)}
            if opt.reg_offset:
                opt.heads.update({"reg": 2})
            if opt.hm_hp:
                opt.heads.update({"hm_hp": max(0, opt.num_joints)})
            if opt.reg_hp_offset:
                opt.heads.update({"hp_offset": 2})
            if getattr(opt, "num_joints", 0) > 0:
                opt.heads.update({"hp_vis": opt.num_joints})
        else:
